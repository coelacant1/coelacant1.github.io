...
function showReleaseLink(show) {
    if (releaseLinkDiv) {
        releaseLinkDiv.style.display = show ? "block" : "none";
    }
}

function updateReleaseLink(url, tagName) {
    const link = document.getElementById("release-url");
    if (link) {
        link.href = url;
...
async function parseHexToBlocks(hexData, blockSize, offset) {
    ...
    // Quick approach: decode to string, then parse line by line.
    // If .hex is extremely large, streaming approach might be needed
